...
impl SearchService {
    pub fn new(cache_path: Option<PathBuf>) -> Self {
        let cache = cache_path.and_then(|path| {
            SearchCache::new(path).ok().map(Arc::new)
        });
    ...
    pub async fn search(&self, query: &str, num_results: usize, providers: Vec<Box<dyn SearchProvider>>) -> Result<Vec<SearchResult>, SearchError> {
        ...
        if providers.is_empty() {
            return Err(SearchError::Config("No search providers configured".to_string()));
        }
        ...
        if valid_provider_results.is_empty() {
            return Err(SearchError::Api("All search providers failed or returned no results".to_string()));
        }

        // 3. Pipeline: Dedup + Rank (RRF)
        let ranked_results = merge_and_rank(valid_provider_results, 60);
        let final_results = ranked_results.into_iter().take(num_results).collect::<Vec<_>>();
