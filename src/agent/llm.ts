...
export async function streamCompletion(
  ...
  const toolsEnabled = !cb.jsonMode && modelSupportsTools(model)
    ...
    const contentToString = (c: LlmMessage['content']): string => {
      ...
      //   3. The last message must never be an assistant message with unresolved
      //      tool_calls (causes "tool must follow tool_calls" on the next request).
      // All three cases can happen after context compression or mid-stream failures.
      const sanitizedMessages = (() => {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (m.role === 'assistant' && m.tool_calls && m.tool_calls.length > 0) {
            ...
            if (hasMissing) {
              console.warn('[sanitize] Dropping incomplete assistant+tool_calls block, missing ids:',
        ...
        const pass2 = pass1.filter(m => {
          ...
          if (!tid || !declaredIds.has(tid)) {
            console.warn('[sanitize] Dropping orphaned tool result, id:', tid)
            return false
          }
          return true
        })
        ...
        // to prevent "tool must follow tool_calls" on the very next LLM request.
        while (pass2.length > 0) {
          ...
          if (last.role === 'assistant' && last.tool_calls && last.tool_calls.length > 0) {
            ...
            if (!hasResults) {
              pass2.pop()
              console.warn('[sanitize] Removed trailing assistant with unresolved tool_calls')
              continue
            }
          }
          break
