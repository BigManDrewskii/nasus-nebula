...
import { JSON_TRUNCATION_LIMIT, CONTENT_TRUNCATION_LIMIT, NAVIGATION_TIMEOUT_MS } from '../lib/constants'

/**
 * Tool execution for the Tauri desktop app.
 *
...
export async function executeTool(
  ...
  switch (toolName) {
    case 'bash': {
      ...
      // Intercept Node.js commands — direct to bash_execute with proper error
      if (/\bnpx\b|\bnpm\b|\bnode\b|\byarn\b|\bpnpm\b|\bbun\b/.test(cmd)) {
        return {
          output:
            'Error: Node.js commands must use bash_execute, not bash. ' +
            'Use bash_execute("npm install ...") for package management.',
          isError: true,
        }
      }

      // Python commands must use python_execute
      if (/\bpip3?\b|\bpython3?\b/.test(cmd)) {
        return {
          output:
            'Error: Python commands must use python_execute for code or bash_execute for pip install. ' +
            'Use bash_execute("pip install ...") for packages.',
          isError: true,
        }
      }

      // curl/wget must use http_fetch or bash_execute
      if (/\bcurl\b|\bwget\b/.test(cmd)) {
        return {
          output:
            'Error: curl/wget must use bash_execute in Docker sandbox. ' +
            'Use bash_execute("curl ...") or http_fetch for HTTP requests.',
          isError: true,
        }
      }

      // apt/brew must use bash_execute
      if (/\bapt\b|\bapt-get\b|\bbrew\b/.test(cmd)) {
        return {
          output:
            'Error: Package managers must use bash_execute in Docker sandbox. ' +
            'Use bash_execute("apt-get install ...") for system packages.',
          isError: true,
        }
      }

      // git must use bash_execute
      if (/\bgit\b/.test(cmd)) {
        return {
          output:
            'Error: git commands must use bash_execute in Docker sandbox. ' +
            'Use bash_execute("git ...") for git operations.',
          isError: true,
        }
      }
      ...
      if (/\bwhich\b/.test(cmd)) {
        return {
          output:
            'Error: Use bash_execute to run shell commands in Docker sandbox. ' +
            'For file operations use write_file, read_file, list_files. ' +
            'For HTTP use http_fetch. For code use python_execute.',
          isError: true,
        }
      }
      ...
      return {
        output:
          'Error: This shell command must use bash_execute in Docker sandbox. ' +
          'Use write_file, read_file, list_files for files. ' +
          'Use http_fetch for HTTP, search_web for search, python_execute for code.',
        isError: true,
      }
    }
    ...
    case 'http_fetch': {
      ...
      try {
        ...
        const status = parseInt(result.slice(0, newlineIdx))
        const text = result.slice(newlineIdx + 1)
        ...
        if (isJson) {
          ...
          const preview = pretty.length > JSON_TRUNCATION_LIMIT ? pretty.slice(0, JSON_TRUNCATION_LIMIT) + '\n[...truncated]' : pretty
          return { output: `HTTP ${status}\n${preview}`, isError: false }
        }

        // Plain text / CSV / other — return as-is with truncation
        const preview = text.length > JSON_TRUNCATION_LIMIT ? text.slice(0, JSON_TRUNCATION_LIMIT) + '\n[...truncated]' : text
        return { output: `HTTP ${status}\n${preview}`, isError: false }
      } catch (err) {
    ...
    case 'serve_preview': {
      ...
      if (cfg.executionMode === 'disabled') {
        return {
          output: 'serve_preview requires Docker. Make sure Docker Desktop is running.',
          isError: true,
        }
      }
