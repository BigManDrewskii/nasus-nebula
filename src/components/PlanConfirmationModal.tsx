...
export const PlanView = memo(({ plan, onApprove, onReject, currentPhase, currentStep, isReadOnly }: PlanViewProps) => {
  ...
  return (
    <div className={`w-full overflow-hidden rounded-[24px] border transition-all duration-700 animate-in fade-in slide-in-from-bottom-8 ${
      isApprovalMode 
        ? 'border-amber-500/40 bg-black/60 shadow-[0_0_50px_rgba(0,0,0,0.5),0_0_20px_rgba(234,179,8,0.05)]' 
      ...
      {/* Header */}
      <div 
        className={`relative px-6 py-4.5 border-b overflow-hidden cursor-pointer flex items-center justify-between gap-6 ${
      ...
      >
        ...
        <div className="flex items-center gap-4 relative z-10 flex-1">
          <div className={`p-2.5 rounded-2xl transition-all duration-500 ${
            ...
            <Pxi name={isApprovalMode ? 'shield-check' : 'cpu'} className={`w-5.5 h-5.5 ${
              isApprovalMode ? 'text-black' : 'text-white/70'
            }`} />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-3">
              <h2 className="text-[14px] font-black font-display text-white tracking-[0.2em] uppercase">
                  {isApprovalMode ? 'Mission Strategy' : 'Execution Engine'}
              </h2>
              {isApprovalMode && (
                  <span className="bg-amber-500/20 text-amber-500 text-[9px] font-black px-1.5 py-0.5 rounded-md border border-amber-500/20 tracking-widest animate-pulse">
                      PENDING APPROVAL
                  </span>
              )}
              {!isApprovalMode && !isCollapsed && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-amber-500/50 uppercase tracking-[0.2em]">Live Stream</span>
                </div>
              )}
            </div>
            <p className="text-[11px] text-white/40 font-bold uppercase tracking-[0.25em] mt-1 line-clamp-1">
              {plan.title}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div className={`p-2 rounded-xl transition-all duration-300 ${isCollapsed ? 'bg-white/5' : 'bg-transparent'}`}>
        ...
        {!isApprovalMode && (
          <div className="absolute bottom-0 left-0 h-[2px] bg-amber-500/20 w-full overflow-hidden">
            <div 
              className="h-full bg-amber-500 transition-all duration-1000 ease-out shadow-[0_0_8px_rgba(234,179,8,0.5)]"
        ...
        <>
          {isApprovalMode && plan.description && (
            <div className="px-6 pt-5">
              <p className="text-[13px] text-white/70 leading-relaxed font-medium bg-black/40 p-4 rounded-2xl border border-white/5 shadow-inner">
          ...
          {/* Plan Content */}
          <div className={`max-h-[400px] overflow-y-auto p-5 space-y-4 custom-scrollbar transition-colors duration-700 ${
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-5">
              <div className="flex items-center gap-6">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.25em] mb-1">System Metrics</span>
                  <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1.5">
                        <Pxi name="zap" className="w-3 h-3 text-amber-500/40" />
                        <span className="text-[11px] font-black text-white/80 font-numeric tracking-widest">{totalSteps} NODES</span>
                      </div>
                      <div className="w-1 h-1 rounded-full bg-white/10" />
                      {plan.estimatedSteps > 0 && (
                        <div className="flex items-center gap-1.5">
                          <Pxi name="timer" className="w-3 h-3 text-amber-500/40" />
                          <span className="text-[11px] font-black text-amber-500/70 font-numeric tracking-widest">~{plan.estimatedSteps}SEC</span>
                        </div>
                      )}
                  </div>
                </div>
              ...
              {isApprovalMode ? (
                <div className="flex gap-3 w-full sm:w-auto">
                  <button
                    onClick={onReject}
                    className="flex-1 sm:flex-none group relative px-6 py-2.5 text-[11px] font-black uppercase tracking-[0.25em] rounded-xl border border-white/10 text-white/40 hover:text-white/90 hover:bg-white/[0.05] hover:border-white/20 transition-all active:scale-95 overflow-hidden"
                  >
                    <span className="relative z-10">REJECT</span>
                  </button>
                  <button
                    onClick={onApprove}
                    className="flex-1 sm:flex-none group relative px-8 py-2.5 text-[11px] font-black uppercase tracking-[0.25em] rounded-xl bg-amber-500 text-black hover:bg-amber-400 transition-all active:scale-[0.98] shadow-[0_10px_30px_rgba(234,179,8,0.2)] hover:shadow-[0_15px_40px_rgba(234,179,8,0.4)] flex items-center justify-center gap-2.5 overflow-hidden"
                  >
                ...
                <div className="flex items-center gap-3.5 py-2 px-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 shadow-[inset_0_0_15px_rgba(234,179,8,0.05)]">
                  <div className="flex gap-1.5">
                    <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping" />
                    <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                  </div>
                  <span className="text-[10px] font-black text-amber-500 uppercase tracking-[0.4em] translate-y-px">DEPLOYED</span>
                </div>
              )}
            </div>
          </div>
