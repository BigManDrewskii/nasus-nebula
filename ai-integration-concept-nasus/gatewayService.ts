...
export class GatewayService {
   ...
   *
   * @param callFn - Function that makes the actual LLM call given (apiBase, apiKey, headers)
   * @returns The result from callFn, plus metadata about which gateway was used
   */
  async callWithFailover<T>(
    callFn: (apiBase: string, apiKey: string, extraHeaders: Record<string, string>) => Promise<T>,
  ...
  private buildHeaders(gw: GatewayConfig): Record<string, string> {
    ...
    if (gw.type === 'openrouter') {
      ...
      // Enable OpenRouter's native provider fallback
      headers['X-Allow-Fallbacks'] = 'true'
    }
